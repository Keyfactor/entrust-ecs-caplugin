...
namespace Keyfactor.Extensions.CAPlugin.Entrust.Models
{
	public class EntrustCertType
	{
		...
		/// </summary>
		public static new List<EntrustCertType> AllTypes = new List<EntrustCertType>() { Standard, Advantage, UC, EV, QWAC, PSD2, Wildcard, Private, PD, CodeSigning, EVCodeSigning, CDSIndividual, CDSGroup, CDSEntLite, CDSEntPro, SMIMEEnt };

		/// <summary>
		/// The certificate types that are available through FLEX inventory.
		/// </summary>
		private static List<EntrustCertType> FlexTypes => AllTypes.Where(x => x.ShortName.Contains("SSL") && x.ShortName != "PD_SSL").ToList();
		...
		/// <returns></returns>
		public static bool InventoryExists(ECSClient client, string productType)
		{
		...
		/// <param name="productType">The product type we seek to check the inventory of.</param>
		/// <returns></returns>
		public static bool ProductIDValid(ECSClient client, string productType)
		{
			...
			EntrustCertType inventory = GetCustomerAccountTypes(client)
		...
		/// <returns></returns>
		public static List<EntrustCertType> GetCustomerAccountTypes(ECSClient client)
		{
			if (client == null)
			{
			...
			// Add the base types.
			List<EntrustCertType> customerTypes = AllTypes
